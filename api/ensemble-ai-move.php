...
/**
 * Ensemble AI Move Endpoint
 * Queries ALL configured LLM providers and combines their reasoning
 * Used for hard mode - combines local AI + multiple LLMs
 */
...
$localAIMove = $input['local_ai_move'] ?? null; // Local AI's suggestion
...
$configuredProviders = getAllConfiguredProviders();
...
// Build prompt
$prompt = buildEnsemblePrompt($board, $gameState, $playerSide, $dbInsights, $localAIMove);
...
foreach ($configuredProviders as $providerKey => $config) {
    $ch = initializeCurlRequest($config, $prompt);
    curl_multi_add_handle($multiHandle, $ch);
    $curlHandles[$providerKey] = ['handle' => $ch, 'config' => $config];
}
...
// Collect responses
foreach ($curlHandles as $providerKey => $data) {
    $ch = $data['handle'];
    $config = $data['config'];
    ...
    if ($httpCode === 200 && $response) {
        try {
            $parsedResponse = parseProviderResponse($config, $response);
            if ($parsedResponse && validateMove($parsedResponse, $board, $gameState, $playerSide)) {
                $llmResponses[] = [
                    'provider' => $config['name'],
                    'move' => $parsedResponse,
                    'confidence' => $parsedResponse['confidence'] ?? 75,
                    'reasoning' => $parsedResponse['reasoning'] ?? 'Strategic move'
                ];
            }
        } catch (Exception $e) {
            error_log("Provider {$config['name']} error: " . $e->getMessage());
        }
    }
...
// If no valid LLM responses, fall back
if (empty($llmResponses)) {
    echo json_encode([
        'success' => false,
        'message' => 'No valid responses from LLM providers',
        'fallback' => true
    ]);
    exit;
}

// Ensemble decision making - combine all LLM suggestions + local AI
$finalMove = ensembleDecision($llmResponses, $localAIMove, $board, $gameState, $playerSide);

echo json_encode([
    'success' => true,
    'move' => $finalMove['move'],
    'reasoning' => $finalMove['reasoning'],
    'confidence' => $finalMove['confidence'],
    'ensemble' => [
        'llm_count' => count($llmResponses),
        'providers' => array_column($llmResponses, 'provider'),
        'consensus' => $finalMove['consensus']
    ]
]);
...
/**
 * Initialize CURL request for a provider
 */
function initializeCurlRequest($config, $prompt) {
    $ch = curl_init();
    ...
    if ($config['type'] === 'claude') {
        ...
        $payload = [
            'model' => $config['model'],
            'messages' => [
                ['role' => 'system', 'content' => 'You are an expert Tactical Pebble Game AI. Analyze board positions and suggest optimal moves. Always respond with valid JSON only.'],
 ...
 */
function parseProviderResponse($config, $response) {
...
/**
 * Ensemble decision making - combine all suggestions
 */
function ensembleDecision($llmResponses, $localAIMove, $board, $gameState, $playerSide) {
    // Count vote for each position
    $positionVotes = [];
    $allMoves = [];

    // Add LLM votes
    foreach ($llmResponses as $response) {
        $move = $response['move'];
        $position = $move['to_position'] ?? -1;

        if ($position >= 0 && $position <= 8) {
            if (!isset($positionVotes[$position])) {
            ...
            $positionVotes[$position]['count']++;
            $positionVotes[$position]['confidence_sum'] += $response['confidence'];
            $positionVotes[$position]['reasons'][] = $response['reasoning'];
            $positionVotes[$position]['providers'][] = $response['provider'];
        }
    ...
    // Add local AI vote with weight
    if ($localAIMove && isset($localAIMove['to_position'])) {
        $position = $localAIMove['to_position'];
        ...
        // Local AI gets 1 vote but with high confidence
        $positionVotes[$position]['count']++;
        $positionVotes[$position]['confidence_sum'] += 80;
        $positionVotes[$position]['reasons'][] = 'Local AI strategic analysis';
        $positionVotes[$position]['providers'][] = 'Local AI';
    }

    // Find the move with highest consensus
    $bestPosition = null;
    $bestScore = -1;

    foreach ($positionVotes as $position => $data) {
        // Score = vote count * average confidence
        $avgConfidence = $data['confidence_sum'] / $data['count'];
        $score = $data['count'] * $avgConfidence;
    ...
    $chosenVote = $positionVotes[$bestPosition];
    $consensusPercentage = round(($chosenVote['count'] / (count($llmResponses) + 1)) * 100);

    return [
        'move' => $chosenVote['move'],
        'reasoning' => "Ensemble consensus ({$consensusPercentage}%): " . implode('; ', array_slice($chosenVote['reasons'], 0, 2)),
        'confidence' => round($chosenVote['confidence_sum'] / $chosenVote['count']),
        'consensus' => [
            'position' => $bestPosition,
            'votes' => $chosenVote['count'],
            'total_voters' => count($llmResponses) + 1,
            'percentage' => $consensusPercentage,
            'providers' => $chosenVote['providers']
        ]
    ];
}

/**
 * Build enhanced prompt for ensemble reasoning
 */
function buildEnsemblePrompt($board, $gameState, $playerSide, $dbInsights = [], $localAIMove = null) {
    ...
    $prompt = "You are part of an AI ensemble analyzing a Tactical Pebble Game position.\n\n";
    $prompt .= "**GAME RULES:**\n";
    ...
    // Add local AI suggestion
    if ($localAIMove && isset($localAIMove['to_position'])) {
        $prompt .= "**LOCAL AI SUGGESTION:**\n";
        $prompt .= "Position: {$localAIMove['to_position']}\n";
        if (isset($localAIMove['reasoning'])) {
            $prompt .= "Reasoning: {$localAIMove['reasoning']}\n";
        }
        $prompt .= "\nYou may agree or suggest a better move.\n\n";
    }
    ...
    $prompt .= "    \"reasoning\": \"Your strategic analysis\",\n";
    $prompt .= "    \"confidence\": <0-100>\n";
    $prompt .= "}\n\nProvide your best move analysis as JSON only.";

    return $prompt;
}

// Copy helper functions from llm-move.php
function getGameInsights($conn, $sessionId, $playerId) {
    ...
    try {
        ...
        error_log("Ensemble insights error: " . $e->getMessage());
    }
